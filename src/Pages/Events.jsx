...
function Events() {
  ...
  const filteredEvents = eventData.filter((event) =>
    event.EventName.toLowerCase().includes(searchTerm) || event.Winner1.toLowerCase().includes(searchTerm) || event.Winner2.toLowerCase().includes(searchTerm) || event.Winner3.toLowerCase().includes(searchTerm) || event.EventDate.toLowerCase().includes(searchTerm) || event.EventStage.toLowerCase().includes(searchTerm) || event.EventState.toLowerCase().includes(searchTerm))
      ...
      <div className="event_box">
        {filteredEvents.map((row, index) => (
          <div className="event_card" key={index}>
            ...
            <span className="eventStage" data-state={row.EventStage}>Stage: {row.EventStage}</span>
            <span className="eventStartTime"> {row.EventStage}</span>
